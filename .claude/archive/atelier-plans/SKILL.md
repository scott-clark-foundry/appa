---
name: atelier-plans
description: Author RFCs, ADRs, technical specs, design docs, vision.md, architecture docs, sprint plans, and debugging walkthroughs as styled, editable HTML instead of Markdown. Output is one self-contained HTML file the user opens in a browser: edit prose inline, tick task checkboxes, cycle status pills, and copy back via Copy diff (paste into Claude Code for iteration) or Copy markdown (finalize as .md with YAML frontmatter). Uses a tight component vocabulary (Goals/Non-goals/Invariants id-lists, callouts, ADR decision blocks, journey scenarios, Mermaid diagrams) in a warm-parchment visual system. Use whenever a user asks to draft, write, or plan an RFC, spec, ADR, design doc, architecture doc, vision doc, sprint plan, or debugging walkthrough; or whenever a plan has ≥3 sections, contains diagrams, has stable IDs (G1, NG2, I3) other paragraphs reference, or will be reviewed iteratively. Skip for one-shot scratch notes, short answers, or single-paragraph docs: Markdown is better.
license: MIT
---

# Atelier Plans

Author non-trivial planning documents as styled, editable HTML instead of Markdown. The output is one self-contained HTML file the user opens in a browser. They can edit prose inline, tick task boxes, cycle statuses, copy the result back to chat as a paste-back diff or finalize as Markdown with YAML frontmatter, and print to PDF.
...
Reach for an HTML plan when **any** of these apply:

- The user names a document type: RFC, ADR, spec, design doc, architecture doc, vision.md, sprint plan, debugging walkthrough
- ≥ 3 sections: long enough that scannability matters
- Contains diagrams, sequence flows, state machines, or scenario walkthroughs
- Has Goals/Non-goals/Invariants with stable IDs (G1, NG2, I3) that other paragraphs reference
- Has tasks, decisions, risks, or acceptance criteria the user will tick off
- Will be revised iteratively: the user edits it in the browser without re-prompting
- Will be shared, reviewed, or printed

**Skip this skill for** one-shot scratch notes, short answers, single-paragraph documentation, README updates, or docstrings. Plain Markdown is better there.
...
2. **Copy the template.** `cp assets/plan-template.html <project>/plans/<area>/<slug>.html`. The template is **self-contained**: CSS and runtime are inlined in `<style id="plan-css-inline">` and `<script id="plan-js-inline">` blocks at the top. No build step, no shared assets to fix.
...
7. **Hand off.** Tell the user the file path and that they can open it in any browser. Mention the two copy primitives explicitly the first time (see next section).
...
- **Copy markdown**: finalizes the plan as `.md` with YAML frontmatter when the user is ready to commit it to the repository as Markdown. They typically use this once the plan is approved.
